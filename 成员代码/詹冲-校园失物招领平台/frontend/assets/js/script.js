...
async function handleLogin(e) {
    ...
    if (captchaInput && captchaDisplay) {
        if (captchaInput.value.trim().toLowerCase() !== (captchaDisplay.dataset.answer || '').toLowerCase()) {
            ...
            captchaDisplay.click && captchaDisplay.click();
            return;
        }
    }
    try {
...
function renderFilters(type) {
    ...
    categoryContainer.innerHTML = state.itemCategories.map(cat => `
        <div class="filter-options-item">
            ...
            <label for="filter-${type}-${cat}">${cat}</label>
        </div>
    `).join('');
}
...
/* =========================
   个人中心地图选点
   ========================= */
function initializeEditMapModal() {
        ...
        <form id="publish-form">
            ...
            <div class="input-group">
                ...
                <div class="relative flex items-center">
                    <input type="text" id="publish-location" name="location" class="input-group-input input-with-action" placeholder="可手动输入或从地图选择">
...
function renderVolunteerItems(type, items) {
    ...
    listElement.innerHTML = items.map(item => {
        ...
        return `
        <div class="item-card item-card-elevated relative">
            ${statusBadge}
            ${isMyItem ? '<div class="my-item-badge">我发布的</div>' : ''}
...
async function handleAIMatch(itemID) { 
    ...
    listElement.innerHTML = '<div class="text-center p-4">正在调用模型进行匹配，请稍候...</div>';
